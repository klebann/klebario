...
CFood::CFood(void)
{
	float _radius(float(std::rand() % 7) + 3);
	int _mass(_radius);
	m_radius = _radius;
	m_mass = _mass;
...
bool CFood::kolizja(CPlayer* _player)
{
	//zbieram pozycje
	...
	{
		float new_radius = sqrt(_player->getRadius() * _player->getRadius() + this->getRadius() * this->getRadius());
		int new_mass = _player->getMass() + this->getMass();

		_player->setRadius(new_radius);
		_player->setMass(new_mass);
		return true;
	} else
		return false;
}